 ...
 */
  ...
  signOut = async () => {
    // var idToken = localStorage.getItem('CognitoIdentityServiceProvider.dllb69draepao1p6sqplv71r.billysun.idToken');
    // var accessToken = localStorage.getItem('CognitoIdentityServiceProvider.dllb69draepao1p6sqplv71r.billysun.accessToken');
    var username = localStorage.getItem('CognitoIdentityServiceProvider.dllb69draepao1p6sqplv71r.LastAuthUser');
    clearCookiesResponse = await _fetch('/authn/sign-out' , {});
    var userData = {
      Username: username,
      Pool: userPool,
    };
  ...
  signIn = async () => {
      ...
      if(flow === 'password'){ //sign-in using password only
        ...
        **/
        ...
        // cognitoUser.setAuthenticationFlowType('DEVICE_SRP_AUTH');
        
        // cognitoUser.authenticateUserInternal(authenticationDetails, authCallBack);
  ...
  authCallBack = {
    ...
    mfaSetup: async function(challengeName, challengeParameters) {
      console.log("mfaSetup:" + challengeName);
      console.log(challengeParameters);
      cognitoUser.associateSoftwareToken(authCallBack);
    },
    associateSecretCode:  async function(secretCode) {
      console.log(secretCode);

      qrString = "otpauth://totp/SCQ:billysun@amazon.com?secret="+secretCode+"&issuer=SCQ";

      var totpCode = prompt("Please enter totp code: " + qrString);

      cognitoUser.verifySoftwareToken(totpCode, "SSSSS", authCallBack);
      
    },
    totpRequired: async function(challengeName, challengeParameters) {
      console.log("totpRequired:" + challengeName);
      console.log(challengeParameters);

      // cognitoUser.associateSoftwareToken(authCallBack);
      
      var totpCode = prompt("Please enter totp code: " + challengeParameters.FRIENDLY_DEVICE_NAME);
      cognitoUser.sendMFACode(totpCode, authCallBack, challengeName);
    },
